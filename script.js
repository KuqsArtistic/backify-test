...
let splatColors = [{r: 0.15, g: 0, b: 0.04249999999999998}];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (frequencyData[0] > 5) return;

    let bass = 0.0;
    let half = Math.floor(frequencyData.length / 2);

    for (let i = 0; i <= config.FREQ_RANGE; i++) {
        bass += frequencyData[i + config.FREQ_RANGE_START];
        bass += frequencyData[half + (i + config.FREQ_RANGE_START)];
    }
